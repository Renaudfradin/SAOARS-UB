...
export default function contentCharacters(){
  ...
  useEffect(() => {
    axios.get('https://api-saoars.vercel.app/perso')
      .then((response)=> {
        //console.log("response api",response.data.characters);
    ...
    <div>
      ...
      {characters.map((character) => (
        <div key={character.id} style={{border:"1px solid white", marginTop:"10px"}}>
          ...
          <br />
          <img src={renderStarCharacter(character.stars)} alt="" />

          <p>{character.weapon_type}</p>
          <p>{character.character_type}</p>

          <img src={renderTypeAtk(character.atk1_type)} alt="" />
          <img src={renderTypeAtk(character.atk2_type)} alt="" />
          <img src={renderTypeAtk(character.atk3_type)} alt="" />
